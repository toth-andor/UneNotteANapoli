 ...
 */
public class CommandLineRunnable {

    private final ICommandLineInterpreter parser;
    private final IController controller;

    public CommandLineRunnable(ICommandLineInterpreter parser, IController controller) {
        this.parser = parser;
    ...
    public static void main(String[] args) {
        ...
        CommandLineRunnable app = new CommandLineRunnable(interpreter, controller);
        app.run();
    }
    ...
    private void displayMapInit() {
        ...
        for(Road r : controller.getMapModel().getRoads()) {
            System.out.println("  #" + r.getName() + " BETWEEN " + r.getEnd1().getName() + " AND " + r.getEnd2().getName());

            for(Lane l : r.getLanes()) {
                System.out.println("    * " + l.getName() + " SRC: " + l.getSource().getName() + " DST: " + l.getDestination().getName() + " " + l.getDirection());
            }
        }
    ...
    private void displayMap(Player current) {
        ...
        System.out.println("Jelenlegi sáv: " + currentLane.getName() + "  " + laneStateDisplay(currentLane) + " " + currentLane.getDirection());
        System.out.println("--------------------------------------------");
        ...
        for (Road road : nextJunction.getRoads()) {
            ...
            for (Lane lane : road.getLanes()) {
                if((lane.getDirection().equals(currentLane.getDirection()) && !(lane.getRoad().equals(currentLane.getRoad())) || !(lane.getDirection().equals(currentLane.getDirection())) && lane.getRoad().equals(currentLane.getRoad()))) {
                    String laneType = lane instanceof OutdoorLane ? "OL" : "TL";
                    System.out.println("\t #" + lane.getName() + "  [" + laneType + "] " + laneStateDisplay(lane) + " SRC: " + lane.getSource().getName() + " DST: " + lane.getDestination().getName() + " " + lane.getDirection());
                }
            }
        }
        System.out.println();
